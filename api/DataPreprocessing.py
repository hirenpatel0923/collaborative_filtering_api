...
class DataPreprocessing():
    def __init__(self):
        ...
        self.max_movieId = 131262
        self.max_userId = 138493
        self.k_factor = 128
        self.seed = 140203
    ...
    def load_data(self, shuffle=True):
        movie_csv = pd.read_csv(self.dataParams.MOVIE_CSV)
        link_csv = pd.read_csv(self.dataParams.LINK_CSV)
        userIds_csv = pd.read_csv(self.dataParams.USERIDS_CSV)
        return movie_csv, link_csv, userIds_csv
        # rating_csv = pd.read_csv(self.dataParams.RATINGS_CSV)
